...
export default function AdminUsers() {
  ...
  const { data: users, refetch, isLoading, error } = trpc.users.getAllUsers.useQuery({ limit: 100, offset: 0 });
  ...
  const handleToggleBlock = async (user: any) => {
    try {
      if (user.isBlocked) {
        ...
        toast.success(`User ${user.username} telah diaktifkan kembali`);
      } else {
        await blockMutation.mutateAsync({ userId: user.id });
        toast.error(`User ${user.username} telah diblokir`);
      }
      refetch();
    } catch (error: unknown) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div>
            ...
            <p className="text-sm text-slate-500">Kelola akses dan status akun pelanggan</p>
          </div>
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <h4 className="font-bold text-slate-800">{user.username || "Unknown"}</h4>
                              {user.role === 'admin' && (
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-3 text-slate-500">
                          ...
                          <span className="text-xs font-medium truncate">{user.email || 'Tidak ada email'}</span>
                        </div>
                        <div className="flex items-center gap-3 text-slate-500">
                        ...
                        <div className="flex items-center gap-3 text-slate-400 pt-3 border-t border-slate-50">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest">
                            Joined {new Date(user.createdAt).toLocaleDateString('id-ID', { month: 'short', year: 'numeric' })}
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        {filteredUsers.length === 0 && (
          <div className="bg-white rounded-3xl p-20 border border-dashed border-slate-200 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold text-slate-800">Tidak ada pengguna ditemukan</h3>
