...
export default function AdminProducts() {
  ...
  const { data: products, refetch } = trpc.products.list.useQuery({ isActive: undefined });
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      setNewProduct({ name: "", slug: "", description: "", price: 0, stock: 0, spiceLevel: 1, imageUrl: "" });
      refetch();
    } catch (error: unknown) {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      await updateMutation.mutateAsync({ id: editProduct.id, ...editProduct });
      ...
      <div className="space-y-8">
          ...
          <CardContent className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-2 lg:col-span-1">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Level Pedas</label>
                <select
                  value={newProduct.spiceLevel}
                  onChange={(e) => setNewProduct({ ...newProduct, spiceLevel: parseInt(e.target.value) })}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all font-medium"
                >
                  {[1, 2, 3, 4, 5].map((level) => (
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Gambar</label>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 flex gap-2">
                    ...
                    >
                      {isUploading ? "..." : newProduct.imageUrl ? "Ganti" : "Pilih"}
                    </Button>
                    {newProduct.imageUrl && (
                      <Button
                        type="button"
            ...
            >
              {createMutation.isPending ? "Sedang Menyimpan..." : "Simpan Produk ke Katalog"}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-sm overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between border-b border-slate-100 p-8">
            <div>
              <CardTitle className="text-xl font-bold text-slate-800">Daftar Produk</CardTitle>
              <p className="text-sm text-slate-500 mt-1">Total {filteredProducts.length} produk aktif</p>
            </div>
            ...
            <Card className="w-full max-w-2xl border-none shadow-2xl overflow-hidden rounded-3xl">
              ...
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Pedas</label>
                    <select
                      value={editProduct.spiceLevel}
                      onChange={(e) => setEditProduct({ ...editProduct, spiceLevel: parseInt(e.target.value) })}
