...
export default function AdminReports() {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `laporan_sambalco_${new Date().toISOString().split("T")[0]}.csv`
    );
        ...
        <div className="flex justify-between items-center bg-slate-900 p-8 rounded-[2rem] text-white shadow-2xl shadow-slate-200">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Pantau performa penjualan dan stok secara real-time
            </p>
          </div>
          <Button
            onClick={exportToCSV}
              ...
              <Card className="border-none shadow-sm overflow-hidden bg-white">
                ...
                <CardContent>
                  <div className="text-3xl font-black text-slate-800">
                    Rp {stats?.monthRevenue?.toLocaleString("id-ID") || 0}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  stats.lowStockProducts.length > 0 ? (
                    stats.lowStockProducts.map((p: { id: number; name: string; stock: number }) => (
                      <div
                        key={p.id}
                        className="p-4 rounded-2xl bg-amber-50 border border-amber-100 flex items-center justify-between"
                      >
