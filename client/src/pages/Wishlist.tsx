...
export default function Wishlist() {
  ...
  if (authLoading) return <div className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto" /></div>;
  ...
  const handleAddToCart = (product: any) => {
    addItem(product.id, product.name, product.price, product.imageUrl, 1);
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg shadow p-12 text-center border">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Wishlist Anda Kosong</h2>
          <p className="text-gray-500 mb-6">Anda belum menambahkan produk apapun ke wishlist.</p>
          <Link href="/catalog">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {wishlistItems.map((item) => (
            <div key={item.id} className="bg-white rounded-lg shadow-sm border p-4 flex flex-col sm:flex-row gap-4 items-center">
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-red-600 font-bold mt-1">{formatRupiah(item.product.price)}</p>
                <p className="text-sm text-gray-500 mt-1">
                  Stok: {item.product.stock > 0 ? `${item.product.stock} tersedia` : <span className="text-red-500">Habis</span>}
                </p>
              </div>
              <div className="flex flex-col gap-2 w-full sm:w-auto">
                ...
                >
                  {removeMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Hapus"}
                </Button>
              </div>
            </div>
          ))}
